...
if ($pdo instanceof PDO && $token !== '') {
    try {
        ...
        shop_log('error', '校验重置密码 token 失败', ['message' => $exception->getMessage()]);
...
<main class="page-shell auth-page">
    <section class="auth-card auth-card--wide">
        ...
        <?php else: ?>
            <div class="auth-description">如果链接已失效，请返回找回密码页面重新生成新的重置链接。</div>
        <?php endif; ?>

        <div class="auth-links">
            <a class="auth-link" href="index.php?page=forgot_password">重新申请链接</a>
        </div>
    </section>
</main>
